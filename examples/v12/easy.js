...
client.on('message', async message => {
    ...
    switch(args[0].toLowerCase()){
        case 'play':
            if(!args[1]) return message.channel.send(`Please specify a stream URL`);
            if(!args[1].startsWith("https://") && !args[1].startsWith("http://")) return message.channel.send(`Wait a minute! That's not a URL!`);
            if(!message.member.voice.channel) return message.channel.send(`You need to join a voice channel before you can play something`);
            ...
            player.play(args[1], {
                ...
                message.channel.send(`There was an error while trying to play your cool song!`);
                console.log(err);
            });
            player.on('play', () => {
                audioplayers.set(message.guild.id, player);
                message.channel.send(`I'm playing your cool song!`);
            });
            player.on('stop', () => {
                audioplayers.delete(message.guild.id, player);
                message.channel.send(`Your song is done!`);
            });
            break;
        case 'destroy':
            if(!message.member.voice.channel) return message.channel.send(`You need to join a voice channel before you can destroy the player`);
            ...
            break;
        case 'disconnect':
            if(!message.member.voice.channel) return message.channel.send(`You need to join a voice channel before you can disconnect the player`);
            if(audioplayers.get(message.guild.id)){
            ...
            break;
        case 'reconnect':
            if(!message.member.voice.channel) return message.channel.send(`You need to join a voice channel before you can reconnect the player`);
            if(audioplayers.get(message.guild.id)) await audioplayers.get(message.guild.id).reconnect(5000);
            break;
        case 'pause':
            if(!message.member.voice.channel) return message.channel.send(`You need to join a voice channel before you can pause the player`);
            if(audioplayers.get(message.guild.id)) audioplayers.get(message.guild.id).pause();
            break;
        case 'resume':
            if(!message.member.voice.channel) return message.channel.send(`You need to join a voice channel before you can resume the player`);
        ...
        case 'volume':
            ...
            break;
        case 'isplayable':
            if(!message.member.voice.channel) return message.channel.send(`You need to join a voice channel before you can see if the song is playable`);
            if(audioplayers.get(message.guild.id)){
            ...
            break;
        case 'listeners':
            if(audioplayers.get(message.guild.id)) message.channel.send(`There are **${String(await audioplayers.get(message.guild.id).getListeners())}** listening in the voice channel!`);
            break;
    }
});

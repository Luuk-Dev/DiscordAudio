...
class AudioManager extends EventEmitter{
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if(globals[channel.id] instanceof ValueSaver){
        ...
        player.play(queue[0].url, {
          ...
          this.emit(`play`, channel, queue[0].url);
  ...
  resume(channel){
    ...
    if(queue[0].paused){
      ...
      queue[0].pauses[queue[0].pauses.length - 1].ended = (new Date()).getTime();
  ...
  isPlaying(channel){
    ...
    return globals[channel.id] instanceof ValueSaver;
